...
A banana tree graph, denoted by $B(n, k)$, is a tree constructed by connecting $n$ star graphs $S_k$ to a root vertex.
TODO: Add graceful labeling definition.
An $\alpha$ labeling is a type of graceful labeling introduced by Rosa~\cite{1} which satisfies the $\alpha$-inequality; there must exist a constant $\alpha$ such that for every edge $uv$, either one of the following holds:
\begin{itemize}
